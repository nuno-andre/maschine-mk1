...
def loop_str(clip):
    ...
    return 'Loop : [{} - {}]'.format(
        bars_to_measure(clip.loop_start, sgnd, sgnum),
        bars_to_measure(clip.loop_end, sgnd, sgnum),
    )
...
class AudioClipEditComponent(CompoundComponent):
    ...
    def _action_loop_inc(self, value):
        ...
        if val >= 0 and val < len(INC_STEPS):
            ...
            self.canonical_parent.timed_message(2, f'Loop Adjust: {INC_DISP[val]}')
            self.canonical_parent.show_message(f'Loop Adjust: {INC_DISP[val]}')
    ...
    def _action_mark_start(self, value):
        if self._scroll_mode:
            ...
            if self.selected_clip_slot and self.selected_clip_slot.has_clip:
                ...
                # bars_to_measure(ls, clip.signature_denominator, clip.signature_numerator)  # noqa
                self.canonical_parent.timed_message(
                    2,
                    'Clip Start: ' + bars_to_measure(ls, clip.signature_denominator, clip.signature_numerator),
                )
    ...
    def _action_mark_end(self, value):
        if self._scroll_mode:
            ...
            if self.selected_clip_slot and self.selected_clip_slot.has_clip:
                ...
                self.canonical_parent.timed_message(
                    2,
                    'Clip End: ' + bars_to_measure(le, clip.signature_denominator, clip.signature_numerator),
                )
    ...
    def _update_clip_name(self):
        ...
        if not cs:
            ...
            self.canonical_parent.send_to_display(f'E<{scene.name}> T:{track.name}', 3)
