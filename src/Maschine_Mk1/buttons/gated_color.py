...
from _Framework.InputControlElement import (
    InputControlElement, MIDI_CC_STATUS, MIDI_NOTE_TYPE,
)
...
class TwinButton(ButtonElement):
    ...
    def __init__(self, is_momentary, channel, partner):
        if not isinstance(partner, ButtonElement):
            raise RuntimeError(f'{partner} is not an instance of ButtonElement')
        super().__init__(is_momentary, MIDI_NOTE_TYPE, channel, partner._original_identifier)
    ...
    def install_connections(self, install_translation, install_mapping, install_forwarding):
        ...
        self._is_mapped = install_mapping(
            self, self._parameter_to_map_to,
