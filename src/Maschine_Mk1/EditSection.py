...
from .MIDI_Map import (
    ...
    QUANT_CONST, QUANT_STRING, REL_KNOB_DOWN, TYPE_TRACK_SESSION, VIEWS_ALL,
)
...
def is_clicked(downtime):
    # clicktime < 500
    return (int(round(time() * 1000)) - downtime) < 500
...
class EditSection(CompoundComponent):
    ...
    def duplciate_clip_slot(self, clip_slot):
        if clip_slot.has_clip:
            try:
                ...
                self.do_message(f'Duplicate Clip {clip_slot.clip.name}')
    ...
    def mod_new_initlen(self, diff):
        ...
        _len = int(self.initial_clip_len)
        self.canonical_parent.timed_message(2, f'Init Clip Len: {_len} Beats', True)
        self.canonical_parent.show_message(f'Initial Clip Length : {_len} beats')

    def mod_quant_size(self, diff):
        self.quantize = max(1, min(len(QUANT_CONST) - 1, self.quantize + diff))
        _quant = QUANT_STRING[self.quantize]
        self.canonical_parent.timed_message(2, f'Quantize: {_quant}', True)
        self.canonical_parent.show_message(f'Quantize set to : {_quant}')
            ...
            if value != 0 and clipslot_component._clip_slot != None:
                ...
                if self.edit_state == ES_DUPLICATE:
                    ...
                    if self.altdown:
                        ...
                        if clip_slot.clip:
                            self.do_message(f'Delete Clip {clip_slot.clip.name}')
    ...
    def _new_scene(self):
        ...
        self.do_message(f'New Scene {new_scene.name}')
    ...
    def _new_audio_track(self):
        ...
        track = song.view.selected_track
        self.do_message(f'New Audio Track {track.name}')
    ...
    def _new_midi_track(self):
        ...
        track = song.view.selected_track
        self.do_message(f'New MIDI Track {track.name}')
    ...
    def _new_midi_clip(self):
        ...
        if song.view.selected_track.has_midi_input:
            ...
            if clip_slot != None and not clip_slot.has_clip:
                ...
                self.do_message(f'New MIDI Clip {clip_slot.clip.name}')
    ...
    def quantize_scene(self, scene, index, fiftyPerc):
        ...
        for track in tracks:
            ...
            if len(clipslots) >= index and clipslots[index].clip != None:
                clipslots[index].clip.quantize(
                    QUANT_CONST[self.quantize],
                    self.shiftdown and 0.5 or 1.0,
                )
    ...
    def _capture_new_scene(self):
        ...
        scene = self.song().view.selected_scene
        self.do_message(f'Capture to {scene.name}')
    ...
    def _duplicate_selected_scene(self):
        ...
        if scene and sindex >= 0:
            self.do_message(f'Duplicate Scene {scene.name}')
    ...
    def _duplicate_selected_track(self):
        ...
        if track and t_index and t_index[1] == TYPE_TRACK_SESSION:
            tindex = t_index[0]
            self.do_message(f'Dupl. Track {track.name}', f'Duplicate Track {track.name}')
    ...
    def _duplicate_selected_clip(self):
        ...
        if clip_slot != None and clip_slot.clip != None:
            self.do_message(f'Duplicate {clip_slot.clip.name}', f'Duplicate Clip {clip_slot.clip.name}')
    ...
    def duplicate_track_cs(self, clip_slot):
        if clip_slot != None:
            ...
            if track and t_index and t_index[1] == TYPE_TRACK_SESSION:
                tindex = t_index[0]
                self.do_message(f'Duplicate Track {track.name}')
                song.duplicate_track(tindex)

    def clear_automation(self, clip_slot):
        song = self.song()
        if clip_slot != None and clip_slot.clip != None:
            self.do_message(f'Clr.Env. {clip_slot.clip.name}', f'Clear Envelopes {clip_slot.clip.name}')
    ...
    def double_clipslot(self, clip_slot):
        ...
        if clip_slot.clip != None and track.has_midi_input:
            ...
            if clip.length <= 2048.0:
                clip.duplicate_loop()
                self.do_message(f'Dupl. Lp: {int(clip.length / 4)} Bars')
    ...
    def create_new_clip(self, clip_slot):
        ...
        if clip_slot.clip == None and track.has_midi_input:
            ...
            self.canonical_parent.focus_clip_detail()
            self.do_message(f'New Midi Clip {song.view.highlighted_clip_slot.clip.name}')
    ...
    def _double_selected_clip(self):
        ...
        if clip_slot != None and clip_slot.clip != None and song.view.selected_track.has_midi_input:
            ...
            if clip.length <= 2048.0:
                ...
                self.do_message(f'Dupl. Lp {int(clip_slot.clip.length / 4)} Bars')
                song.view.detail_clip = clip_slot.clip
                self.canonical_parent.focus_clip_detail()
            else:
                self.do_message('Clip is to long to Duplicate')
    ...
    def _select(self, button):
        ...
        self.action_time = int(round(time() * 1000))
