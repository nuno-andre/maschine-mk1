...
from _Framework.InputControlElement import MIDI_CC_TYPE, MIDI_NOTE_OFF_STATUS, MIDI_NOTE_ON_STATUS
...
class PadMode(MaschineMode):
    ...
    def inc_scale(self, inc):
        ...
        if prev_value != self.current_scale_index:
            ...
            self.canonical_parent.show_message(
                f' PAD Scale {newscale.name} {BASE_NOTE[self._base_note]}{newscale.to_octave(self._octave) - 2}')
            self.text_current_scale()
            self.update_transpose()
            if self._seg_display:
    ...
    def inc_octave(self, inc):
        ...
        scale = SCALES[self.current_scale_index]
        self.canonical_parent.show_message(f' OCTAVE {BASE_NOTE[self._base_note]}{newoctave - 2} to {scale.name}')
        self.text_current_scale()
        self.update_transpose()
        if self._seg_display:
            val = newoctave - 2
    ...
    def assign_edit_mono(self, in_notes, button, note_value):
        if in_notes and note_value in in_notes:
