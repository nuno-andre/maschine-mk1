...
class ControlMode(MaschineMode):
    ...
    def __init__(self, button_index, mono_chrome=False, *args, **kwargs):
        ...
        self.track_selections = tuple(ControlTrackElement(i) for i in range(8))
        self._is_monochrome = mono_chrome
        self.main_buttons = (
            [None, self._do_select_level],
    ...
    def navigate(self, dir, modifier, alt_modifier=False):
        ...
        if new_off >= 0 and new_off + 8 <= nr_of_tracks:
            ...
            self.canonical_parent.show_message(f'Control assigned to Tracks {self._track_off + 1} to {self._track_off + 9}')
            self._assign(False)
    ...
    def _mode_sel(self):
        self.main_buttons[0][0].send_color_direct(
            self._mode == CM_LEVEL and PColor.MIX_MODE_VOLUME[0] or PColor.MIX_MODE_VOLUME[1])
        self.main_buttons[1][0].send_color_direct(
            self._mode == CM_PAN and PColor.MIX_MODE_PANNING[0] or PColor.MIX_MODE_PANNING[1])
        self.main_buttons[2][0].send_color_direct(
            self._mode == CM_SEND and PColor.MIX_MODE_SEND[0] or PColor.MIX_MODE_SEND[1])
        self.main_buttons[3][0].send_color_direct(
            self._mode == CM_DEVICE and PColor.MIX_MODE_DEVICE[0] or PColor.MIX_MODE_DEVICE[1])
    ...
    def _do_nav1(self, value):
        ...
        if value > 0:
            if self._mode == CM_SEND:
                ...
                if (
                    not self.application().view.is_view_visible('Detail')
                    or not self.application().view.is_view_visible('Detail/DeviceChain')
                ):
