...
from .MIDI_Map import (
    arm_exclusive, register_sender, PColor, BASIC_CHANNEL, COLOR_HUE_NAV,
    FEEDBACK_CHANNELS, ND_KEYBOARD1, PAD_TRANSLATIONS, SENDS, USE_DISPLAY, VIEWS_ALL,
)
from .MIDI_Map import (
    ARM_CC_OFF, DEVICE_CC_OFF, DEVICE_BUTTON_CC_OFF, LEVEL_CC_OFF, MUTE_CC_OFF, PAN_CC_OFF,
    SEND_CC_OFF, SELECT_CC_OFF, SOLO_CC_OFF, STOP_CC_OFF,
)
...
# from _Framework.ControlSurface import _scheduled_method
...
class Maschine(ControlSurface):
    """Basic Control Script for All Maschine Modell Mikro, Mikro Mk2,
    Mk1, Mk2, Studio.
    """
    __module__ = __name__
    ...
    def _set_up_session(self, mode_selector):
        ...
        track_stop_buttons = [StateButton(is_momentary, MIDI_CC_TYPE, BASIC_CHANNEL, i + STOP_CC_OFF) for i in range(4)]
    ...
    def _do_key_color(self, value):
        if 0 < value < 128:
            ...
            raise ValueError(f'value is bigger than 127: {value}')
    ...
    def display_parameters(self, paramlist):  # FIXME
        if USE_DISPLAY is False:
            return
