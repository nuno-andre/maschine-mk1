...
from .MIDI_Map import REL_KNOB_DOWN
...
class NoteRepeatComponent(CompoundComponent):
    ...
    def __init__(self, note_repeat=None, *args, **kwargs):
    ...
    def mod_button(self, button, inc, which):
        ...
        if new_idx != cindex:
            self.canonical_parent.show_message(f'Note Repeat Button {which} : {CFG_REPEAT_DISPLAY[new_idx]}')
