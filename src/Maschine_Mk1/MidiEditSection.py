...
from .MIDI_Map import debug_out, REL_KNOB_DOWN
...
class MidiEditSection(CompoundComponent):
        ...
        if clip_slot and clip_slot.has_clip and clip_slot.clip.is_midi_clip:
            ...
            if len(notes) > 0:
                ...
                for note in notes:
                    ...
                    lst.append((note[0] + dir, note[1], note[2], note[3], note[4]))

                clip.replace_selected_notes(tuple(lst))
    ...
    def execute_split(self):
        ...
        if selected_note:
            ...
            lst = []
            div_list = self.get_interval(notelen, self._split_value, self._bend_val)
            ...
            for index in range(self._split_value):
                ...
                if notvlen > 0.0:
                    lst.append((note, pos, notvlen, vel, mute))
                pos += div
            ...
            self._current_clip.replace_selected_notes(tuple(lst))
    ...
    def _do_fade(self, value):
        ...
        if newvale >= -1.0 and newvale <= 1.0:
            ...
            _vel_fade_str = int(round(self._vel_fade * 100, 0))
            self.canonical_parent.timed_message(2, f'FADE:{_vel_fade_str}%')
            self.canonical_parent.show_message(f'Split Fade: {_vel_fade_str}%')
            self.execute_split()
    ...
    def _do_gate(self, value):
        ...
        if newval > 0.1 and newval <= 1.0:
            self._gate = newval
            _gate_str = int(round(self._gate * 100, 0))
            self.canonical_parent.timed_message(2, f'GATE:{_gate_str}%')
            self.canonical_parent.show_message(f'Split Gate: {_gate_str}%')
            self.execute_split()
    ...
    def _do_bend(self, value):
        ...
        if newval >= -100 and newval <= 100 and self._selected_note:
            ...
            self.canonical_parent.timed_message(2, f'BEND:{self._bend_val}%')
            self.canonical_parent.show_message(f'Split Bend: {self._bend_val}%')
            lst = self.get_interval(self._selected_note[2], self._split_value, self._bend_val)
            self.execute_split()
