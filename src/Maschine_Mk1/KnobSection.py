...
from .MIDI_Map import arm_exclusive, QUANT_CONST, QUANT_STRING, SHIFT_INC
...
QUANT_DESCR = (
    ...
    ' 1/4 Rec Quantize',
    ' 1/8 Rec Quantize',
    ' 1/8 Rec Triplet Quantize',
    '1/8 & 1/8 Triplet Quantize',
    ' 1/16 Rec Quantize',
    ' 1/16 Triplet Rec Quantize',
    ' 1/16 & 1/16 Triplet Rec Quantize',
    '1/32 Rec Quantize',
)
QUANTIZATION_NAMES = (
    '1/4', '1/8', '1/8t', '1/8+t', '1/16', '1/16t', '1/16+t', '1/32',
)
CLIQ_DESCR = (
    'None', '8 Bars', '4 Bars', '2 Bars', '1 Bar', '1/2', '1/2T',
...
class KnobSection(CompoundComponent):
    ...
    def __init__(self, modeselector, editsection, *args, **kwargs):
    ...
    def _do_channel_slider(self, value):
        ...
        if self._editsection.isShiftdown():
            dir_ = (
                value == 127 and Live.Application.Application.View.NavDirection.left
                or Live.Application.Application.View.NavDirection.right
            )
            self.application().view.scroll_view(dir_, 'Detail/DeviceChain', True)
        elif self._push_down:
            dir_ = value == 127 and -1 or 1
            scenes = song.scenes
            scene = song.view.selected_scene
            sindex = list(scenes).index(scene)
            sel_scene = sindex + dir_
    ...
    def chg_volume(self, diff):
        if self._push_down:
            self.song().master_track.mixer_device.volume.value = calc_new_parm(
                self.song().master_track.mixer_device.volume, diff)
        else:
            repeat(self.song().master_track.mixer_device.volume, diff)

    def chg_xfade(self, diff):
        if self._push_down:
            self.song().master_track.mixer_device.crossfader.value = calc_new_parm(
                self.song().master_track.mixer_device.crossfader, diff)
        else:
            repeat(self.song().master_track.mixer_device.crossfader, diff)

    def chg_cue(self, diff):
        if self._push_down:
            self.song().master_track.mixer_device.cue_volume.value = calc_new_parm(
                self.song().master_track.mixer_device.cue_volume, diff)
        else:
